	...
	Для Каждого СтрокаСценария Из СценарииБизнесПроцесса Цикл
		...
		Если ПараметрыФормированияТекстаСценария.ЧтениеИзТекста Тогда
			Если ПустаяСтрока(СтрокаСценария.СценарийНаименование) Тогда
				// ++ 12.05.2021 15:13:25 РоманцовС romantsov_s@rusklimat.ru / /
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Ошибка при компиляции сценария: %1. Пустое наименование сценария для шага %2'"), Процесс, СтрокаСценария.ШагПроцесса);
				// -- 12.05.2021 15:13:25 РоманцовС romantsov_s@rusklimat.ru / /
				Продолжить;
			КонецЕсли;	 
		...
		Иначе	
			...
			Если ПараметрыПодключения <> Неопределено Тогда
				...
				Иначе
					...
					// ++ 20.04.2021 14:38:40 РоманцовС romantsov_s@rusklimat.ru / / //для шагов процесса - вложенных процессов сценарий пустой. Нужно получать исполнителя шага
					Если Не ЗначениеЗаполнено(Сценарий) И ПараметрыДляПолученияТекстаСценария.Свойство("ИсполнительШагаПроцесса")
						И ЗначениеЗаполнено(ПараметрыДляПолученияТекстаСценария.ИсполнительШагаПроцесса) Тогда 
						ИмяПрофиля = ИмяПрофиляTestClient(ПараметрыДляПолученияТекстаСценария.ИсполнительШагаПроцесса, НастройкиБДДляЗапускаТестовИзСценария, Истина);
				КонецЕсли;
					// -- 20.04.2021 14:38:40 РоманцовС romantsov_s@rusklimat.ru / /
				КонецЕсли;	 
