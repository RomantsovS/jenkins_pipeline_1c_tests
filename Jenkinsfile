pipeline {
    ...
    stages {
        ...
        stage("Run ib release update") {
            when { expression {params.run_ib_release_update} }
            ...
            steps {
                script {
                    try { timeout(time: env.TIMEOUT_FOR_COMLILE_TESTS_STAGE.toInteger(), unit: 'MINUTES') {
                        ...
                        if (fileExists("${env.WORKSPACE}/build/compile_log.txt")) {
                            def data = readFile(file: './build/compile_log.txt')
                            println(data)
                        }
                    }}
                    catch (Throwable excp) {
