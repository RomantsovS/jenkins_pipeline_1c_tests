...
Функция Инициализация()
    ...
    Если ЗначениеЗаполнено(ОжидатьВыполнениеОтложенныхОбработчиков) И ОжидатьВыполнениеОтложенныхОбработчиков = "1" Тогда
...
Функция УстановитьВнешнееСоединениеСБазой(АутентификацияОперационнойСистемы = Ложь, ФайловыйВариантРаботы = Ложь)
    ...
    Иначе
        ...
        СтрокаБазы = СтрЗаменить(СтрокаБазы, "&ИмяСервера1СПредприятия", СЕРВЕР + ?(ЗначениеЗаполнено(ПОРТ_КЛАСТЕРА), ":" + ПОРТ_КЛАСТЕРА, ""));
...
Процедура ЗапуститьОтложенноеОбновлениеВИнформационнойБазе(Соединение, ОжидатьЗавершениеВыполнения, ПериодичностьПроверкиСостоянияВыполнения = 300, ТаймаутПроверкиСостоянияВыполнения = 3600)
	...
	ВремяЗапускаОтложенногоОбновления = ТекущаяДатаСеанса();
	...
	Пока Сведения.ОтложенноеОбновлениеЗавершеноУспешно = Неопределено Цикл
		...
		//ОбработчикиОбновления = ПолучитьТаблицуОбработчиков(Сведения);
		
		// если активно, подождем...
		sleep(ПериодичностьПроверкиСостоянияВыполнения);
		
		Если ВремяЗапускаОтложенногоОбновления + ТаймаутПроверкиСостоянияВыполнения < ТекущаяДатаСеанса() Тогда 
			Прервать;
		КонецЕсли;
